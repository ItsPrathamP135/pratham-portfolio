      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {algorithms.map((algo) => (
          <div
            key={algo.id}
            onClick={() =>
              navigate(`/pratham-portfolio/dsa/sorting/${algo.id}`)
            }
            className={`${
              darkMode
          ...
          >
            <h3
              className={`text-xl font-bold mb-3 ${darkMode ? "text-teal-400" : "text-teal-600"}`}
            >
              {algo.name}
            </h3>
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                ...
                <span
                  className={`px-2 py-1 rounded text-xs font-medium ${getDifficultyStyle(algo.difficulty)}`}
                >
                  {algo.difficulty}
                </span>
              </div>
              <div className="flex justify-between items-center">
                ...
                <code
                  className={`text-xs font-mono ${darkMode ? "text-purple-400" : "text-purple-600"}`}
                >
                  {algo.complexity}
                </code>
              </div>
