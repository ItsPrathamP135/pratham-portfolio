...
const QuickSort = () => {
  ...
  const generateSteps = (arr: number[], ascending: boolean) => {
        ...
        if (start === end && !sorted.includes(start)) {
          ...
          allSteps.push(
            makeStep(
              ...
              {},
              `Single element ${w[start]} — already in position`,
            ),
          );
        }
        return;
      ...
      if (ascending) {
        ...
        allSteps.push(
          makeStep(
            ...
            { pivotIndex: pivotIdx },
            `Pass ${pass} — Pivot selected: ${pivot} (index ${pivotIdx})`,
            pass,
          ),
        );
        ...
        for (let j = start; j < end; j++) {
          ...
          allSteps.push(
            makeStep(
              ...
              { pivotIndex: end, comparing: j },
              `Comparing ${w[j]} with pivot ${pivot}`,
              pass,
            ),
          );
          ...
          if (w[j] < pivot) {
            if (i !== j) {
              allSteps.push(
                makeStep(
                  ...
                  { pivotIndex: end, swapping: [i, j] },
                  `${w[j]} < ${pivot} → Swap ${w[j]} & ${w[i]}`,
                  pass,
                ),
              );
              [w[i], w[j]] = [w[j], w[i]];
              ...
              allSteps.push(
                makeStep(
                  ...
                  { pivotIndex: end, left: [j] },
                  `${w[j]} < ${pivot} → already on left`,
                  pass,
                ),
              );
            }
            ...
            allSteps.push(
              makeStep(
                ...
                { pivotIndex: end, right: [j] },
                `${w[j]} >= ${pivot} → stays on right`,
                pass,
              ),
            );
          }
        ...
        allSteps.push(
          makeStep(
            ...
            { pivotIndex: end, swapping: [i, end] },
            `Placing pivot ${pivot} at index ${i}`,
            pass,
          ),
        );
        [w[i], w[end]] = [w[end], w[i]];
        ...
        allSteps.push(
          makeStep(
            ...
            { pivotIndex: i, left: L, right: R, passComplete: true },
            `Pass ${pass} done — Pivot ${pivot} fixed at ${i}. Left:[${L.map((x) => w[x])}] Right:[${R.map((x) => w[x])}]`,
            pass,
          ),
        );
          ...
          if (w[j] < pivot) {
            if (i !== j) {
              ...
              allSteps.push(
                makeStep(
                  ...
                  { pivotIndex: start, right: [j] },
                  `${w[j]} < ${pivot} → already on right`,
                  pass,
                ),
              );
            }
            ...
            allSteps.push(
              makeStep(
                ...
                { pivotIndex: start, left: [j] },
                `${w[j]} >= ${pivot} → stays on left`,
                pass,
              ),
            );
          }
        ...
        allSteps.push(
          makeStep(
            ...
            { pivotIndex: start, swapping: [start, i] },
            `Placing pivot ${pivot} at index ${i}`,
            pass,
          ),
        );
    ...
    allSteps.push(
      makeStep(
        w,
        Array.from({ length: w.length }, (_, i) => i),
        0,
        w.length - 1,
        {},
        `Sorting complete! Final array: [${w.join(", ")}]`,
      ),
    );
  ...
  const getHistoryBoxClass = (step: Step, index: number) => {
    ...
    else if (index < step.rangeStart || index > step.rangeEnd)
      c += " history-inactive";
    return c;
  };
  ...
  return (
    <div className={`container ${darkMode ? "dark" : "light"}`}>
      ...
      <div className="theory-section">
        ...
        <p className="description">
          Quick Sort is a divide-and-conquer algorithm. It picks a pivot,
          partitions the array so smaller elements go left and larger go right,
          fixes the pivot in its final sorted position, then recursively repeats
          on both sides.
        </p>
        <div className="characteristics-grid">
          <div className="card">
            ...
            <ul>
              <li>
                <strong>Best:</strong> O(n log n)
              </li>
              <li>
                <strong>Average:</strong> O(n log n)
              </li>
              <li>
                <strong>Worst:</strong> O(n²) — bad pivot
              </li>
            </ul>
          </div>
          <div className="card">
            ...
            <p>
              <strong>O(log n)</strong> — average (call stack)
            </p>
            <p>
              <strong>O(n)</strong> — worst (deep recursion)
            </p>
          ...
          <div className="card">
            ...
            <p>
              Swapping across array changes relative order of equal elements
            </p>
          </div>
          <div className="card">
          ...
          <>
            ...
            <div className="step-info">
              <div className="step-counter">
                ...
                {s.passNumber && (
                  <span className="badge-pass">Pass {s.passNumber}</span>
                )}
              </div>
      ...
      <div className="visualization-section">
        ...
        <div className="legend">
          <div className="legend-item">
            <span className="legend-box box-inactive"></span>
            <span>Other Partition</span>
          </div>
          <div className="legend-item">
          ...
          <div className="legend-item">
            <span className="legend-box box-pivot"></span>
            <span>Pivot</span>
          </div>
          <div className="legend-item">
          ...
          <div className="legend-item">
            <span className="legend-box box-left"></span>
            <span>Left (smaller)</span>
          </div>
          <div className="legend-item">
            <span className="legend-box box-right"></span>
            <span>Right (greater)</span>
          </div>
          <div className="legend-item">
                ...
                >
                  ...
                  <div className="history-array-display">
                    {step.array.map((value, index) => (
                      <div
                        key={index}
                        className={getHistoryBoxClass(step, index)}
                      >
                        {value}
                      </div>
                    ))}
    ...
    public static void quickSortAscending(int[] arr, int start, int end) {
        ...
        if (end > start) {
            ...
            <div className="code-block">
              ...
              <pre>
                <code>{`public static void quickSortDescending(int[] arr, int start, int end) {
    int temp = 0;
    if (start < end) {
    ...
    // Include quickSortAscending() and quickSortDescending() here
            ...
            <div className="code-block output-block">
              <h3>📤 Sample Output</h3>
              <pre>
                <code>{`Sorted array in ascending order  :- [0, 5, 5, 9, 10, 14, 18, 25, 25, 27, 64, 76, 82]

Sorted array in descending order :- [82, 76, 64, 27, 25, 25, 18, 14, 10, 9, 5, 5, 0]`}</code>
              </pre>
            </div>
          </div>
        )}
        ...
        <div className="steps-explanation">
          <div className="step">
            ...
            <div className="step-content">
              ...
              <p>
                Pick a pivot element. Last element for ascending, first element
                for descending
              </p>
            </div>
          </div>
          <div className="step">
            ...
            <div className="step-content">
              ...
              <p>
                Compare each element with pivot. Smaller elements move left,
                larger stay right
              </p>
            </div>
          </div>
          <div className="step">
            ...
            <div className="step-content">
              <h3>Place Pivot</h3>
              <p>
                Swap pivot into its correct sorted position. It never moves
                again after this
              </p>
            </div>
          </div>
          <div className="step">
            ...
            <div className="step-content">
              ...
              <p>
                Repeat the same process on both subarrays until every element is
                sorted
              </p>
            </div>
          </div>
